...
import { C, PageHeader, ErrorBanner, Spinner, MEMBER_COLORS } from '../components/ui.jsx'
...
function AssetLink({ href, label, icon }) {
  if (!href || href.startsWith('__workflow:')) {
    return (
      <span style={{ fontSize: 12.5, color: '#C8C3BC', fontStyle: 'italic' }}>Not uploaded yet</span>
    )
  }
  return (
    <a href={href} target="_blank" rel="noreferrer" style={{
      display: 'inline-flex', alignItems: 'center', gap: 5,
      color: C.blue, background: C.blueLight, textDecoration: 'none',
      fontSize: 12.5, fontWeight: 600, padding: '4px 10px',
      borderRadius: 7, border: `1px solid ${C.blue}22`,
      transition: 'background 0.1s',
    }}>
      <span>{icon}</span>
      {label}
...
function ProductAssetsPanel({ products, assets }) {
  // Only show for products that are in 'test' status
  const testProducts = products.filter(p => p.status === 'test')
  if (testProducts.length === 0) return null

  // Map assets by product name for O(1) lookup
  const assetMap = {}
  assets.forEach(a => { assetMap[a.product_name] = a })

  return (
    <div style={{ marginTop: 32 }}>
      {/* Section header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: C.text, letterSpacing: '-0.02em' }}>
          🗂️ Product Assets
        </h3>
        <span style={{ fontSize: 12, color: C.textLight, background: C.border, padding: '2px 9px', borderRadius: 10, fontWeight: 600 }}>
          Creative deliverables from Saida
        </span>
        <span style={{ fontSize: 12, color: C.textLight, marginLeft: 4 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
        {testProducts.map(product => {
          const a = assetMap[product.name] || {}
          const hasAnyAsset = a.landing_page_url || a.creatives_drive_url || a.images_drive_url
            ...
            <div key={product.id} style={{
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {product.sku && (
                    <span style={{ fontSize: 11, color: C.textLight, background: C.border, padding: '1px 7px', borderRadius: 6, fontWeight: 600 }}>
                      {product.sku}
                    </span>
                  )}
                </div>
                {hasAnyAsset
                  ? <span style={{ fontSize: 11, fontWeight: 700, color: '#1E7B4B', background: '#EBF7F1', padding: '2px 9px', borderRadius: 10 }}>Has assets</span>
                  : <span style={{ fontSize: 11, fontWeight: 700, color: '#9B9589', background: '#F0EDE8', padding: '2px 9px', borderRadius: 10 }}>Pending</span>
                }
              </div>
              ...
              <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                {/* Landing page */}
                <div>
                  <div style={{ fontSize: 10.5, fontWeight: 700, color: C.textLight, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 5 }}>
                    🌐 Landing Page
                  </div>
                  <AssetLink href={a.landing_page_url} icon="↗" label="Open Landing Page" />
                </div>
                ...
                <div>
                  ...
                  <AssetLink href={a.creatives_drive_url} icon="📁" label="Open Drive Folder" />
                </div>
                ...
                <div>
                  ...
                  <AssetLink href={a.images_drive_url} icon="🖼️" label="Open Image" />
                </div>
              </div>
            </div>
          )
...
export default function ProductsTable() {
  ...
  const { rows: assets } = useRealtimeTable('product_assets', { orderBy: 'created_at', ascending: true })
  ...
  return (
    <div>
      ...
      {/* ── Product Assets ─────────────────────────────────────────────────── */}
      <ProductAssetsPanel products={filtered} assets={assets} />
